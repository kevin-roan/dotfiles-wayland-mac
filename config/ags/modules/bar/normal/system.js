    ...
    Label({
      setup: (self) => self.poll(2000, (self) => {
        Utils.execAsync(['sensors'])
          .then(output => {
            const temp = output.match(/Package id 0:\s+\+(\d+)/)?.[1] || '0';
            self.label = `${temp}°C`;
            // Add/remove warning class based on temperature
            self.toggleClassName('txt-warning', parseInt(temp) >= 80);
          })
          .catch(print);
      }),
    }),
...
const TlpStatus = () => Box({
  className: "spacing-h-4 txt-onSurfaceVariant",
  children: [
    MaterialIcon("power_input", "small"),
    Label({
      setup: (self) => self.poll(5000, (self) => {
        Utils.execAsync(['tlp-stat', '-s'])
          .then(output => {
            // Extract mode from tlp-stat output
            const mode = output.includes('Mode           = AC') ? 'AC' : 'BAT';
            self.label = mode;
            // Update icon and style based on mode
            self.siblings[0].label = mode === 'AC' ? 'power_input' : 'battery_charging';
            self.toggleClassName('txt-warning', mode === 'BAT');
          })
          .catch(print);
      }),
    }),
